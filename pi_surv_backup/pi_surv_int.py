...
# function to test internet presence
def check_internet():
##        print ("[INFO] Checking Internet")
        r=os.system("wget -T 1 -q --spider http://google.com")
##        print (r)
##        print ("[INFO] Done Checking Internet")
        if r == 0:
                print("[INFO] Online")
                return 1
        else:
...
if conf["use_dropbox"] and check_internet():
	# connect to dropbox with access token
	client = DropboxClient(conf["access_token"])
		...
		if (timestamp - lastUploaded).seconds >= conf["min_upload_seconds"]:
			...
			if motionCounter >= conf["min_motion_frames"]:
				...
				if conf["use_dropbox"] and check_internet():
					...
					response = client.put_file(path, open(t.path, "rb"))
